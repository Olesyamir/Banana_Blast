...
public override void Update(GameTime gameTime)
{
    if (Global._screenState==ScreenState.IsGame)
    {
        ...
        foreach (var s in MonstreManager._monstres)
        {
            ...
            if (s._Rect.Bottom >= 730) 
            {
                _ship.playerGotHit(s.getDamage());//les monstres sont passés le joueur perd de la vie
                s.MonsterGotHit(s._Health);
            }
            else if (s._Rect.Intersects(_ship._Rect))
            {
                explosions.Add(new Explosion(exploTexture, s.getPos(),new Vector2(60,60)));
                _ship.playerGotHit(s.getDamage());//les monstres sont passés le joueur perd de la vie
                s.MonsterGotHit(s._Health);
            }
            else
            {
                foreach (var p in _projectiles)
                {
                    if (p._Rect.Intersects(s._Rect))
                    {
                        ...
                        explosions.Add(new Explosion(exploTexture, new Vector2(la- (p._Rect.Width*1.5f),lo- (p._Rect.Height*3f)),new Vector2(60,60)));
                        _killprojectiles.Add(p);
                        s.MonsterGotHit(50);
                    }

                    // Retirer les projectiles qui sortent de l'écran en les enlevant de la 
                    if (p._Rect.Top < 0)
                    {
                        _killprojectiles.Add(p);
                    }
